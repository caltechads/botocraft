...
class TestBotocraftSettings:
    ...
    def test_uses_default_tunnel_settings(self) -> None:
        ...
        assert settings.tunnel.provisioner_tag_key == "Purpose"
        assert settings.tunnel.provisioner_tag_value == "provisioner"
...
""".strip(),
        ...
        assert settings.tunnel.provisioner_tag_key == "Role"
        assert settings.tunnel.provisioner_tag_value == "jump-host"
...
""".strip(),
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "BOTOCRAFT_TUNNEL__PROVISIONER_TAG_KEY": "Purpose",
                "BOTOCRAFT_TUNNEL__PROVISIONER_TAG_VALUE": "provisioner",
            },
            clear=True,
        ):
            settings = BotocraftSettings()
    ...
    def test_uses_explicit_config_file_override(self, tmp_path: Path) -> None:
        ...
        config_file.write_text(
            """
[tunnel]
provisioner_tag_key = "ManagedBy"
provisioner_tag_value = "provisioner-node"
""".strip(),
            encoding="utf-8",
        )
        ...
        assert settings.tunnel.provisioner_tag_key == "ManagedBy"
        assert settings.tunnel.provisioner_tag_value == "provisioner-node"
