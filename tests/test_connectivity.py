...
class TestProvisionerLocator:
    """Verify provisioner instance discovery behavior."""

    def test_uses_configured_tag_filters(self) -> None:
        """Look up provisioners with configurable tag key/value filters."""
        ...
        settings = BotocraftSettings(
            tunnel=TunnelSettings(
                provisioner_tag_key="Role",
                provisioner_tag_value="jump-host",
            )
        )

        with patch("botocraft.services.ec2.Instance") as instance_cls:
            instance_cls.objects.using = using_mock
            locator = ProvisionerLocator(settings=settings, session=session)
...
class TestTunnelAwareConnectionResolver:
    ...
    def test_raises_when_no_provisioner_exists_outside_aws(self) -> None:
        """Raise a clear error only when the tunnel path needs a provisioner."""
        locator = Mock()
    ...
    def test_enters_tunnel_and_returns_localhost_target(self) -> None:
        ...
        locator = Mock()
        locator.find_instance.return_value = provisioner
        resolver = TunnelAwareConnectionResolver(
            settings=BotocraftSettings(
        ...
        with resolver.open_connection_target(
            ...
            assert target.provisioner_instance is provisioner
