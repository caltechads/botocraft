...
* provisioner instance discovery
* common errors
...
* credentials/profile access that can start SSM sessions against the provisioner
  instance
...
* otherwise it resolves a provisioner instance in the target VPC and opens a
...
Provisioner lookup searches for a running EC2 instance in the target VPC with
...
.. code-block:: toml
    ...
    [tunnel]
    enabled = true
    provisioner_tag_key = "Purpose"
    provisioner_tag_value = "provisioner"
...
.. code-block:: bash
    ...
    export BOTOCRAFT_TUNNEL__PROVISIONER_TAG_KEY=Purpose
    export BOTOCRAFT_TUNNEL__PROVISIONER_TAG_VALUE=provisioner
...
``Unable to connect to ... no running provisioner instance matched tag ...``
    Confirm the target VPC contains a running EC2 provisioner instance with the
