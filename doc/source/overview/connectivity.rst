...
* ``provisioner_instance``: EC2 jump host used for the tunnel, when applicable

Why context manager?
--------------------

When ``botocraft`` is running outside AWS, private endpoints may only be
reachable through a provisioner instance inside the target VPC. In that case
...
* missing provisioner instances only fail when a tunnel is actually required
