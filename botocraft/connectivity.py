...
class ResolvedConnectionTarget(AbstractContextManager["ResolvedConnectionTarget"]):
    ...
    Keyword Args:
        ...
        provisioner_instance: EC2 instance used as the tunnel hop when tunneling.
    ...
    tunneled: bool
    #: Provisioner instance used to establish the tunnel, when applicable.
    provisioner_instance: Instance | Any | None
    ...
    def __init__(
        ...
        *,
        tunneled: bool = False,
        provisioner_instance: Instance | Any | None = None,
        ...
        Keyword Args:
            tunneled: Whether this target should open a local tunnel on entry.
            provisioner_instance: EC2 instance used as the tunnel hop when
                tunneling.
        ...
        #: Provisioner instance used to establish the tunnel, when applicable.
        self.provisioner_instance = provisioner_instance
        #: Underlying context manager that opens and closes the tunnel.
...
class TunnelCapabilityValidator:
    """
    Verify that a provisioner instance supports tunnel creation.

    This validator keeps resolver errors focused and user-facing, without leaking
    lower-level attribute errors from the provisioner object.
    """

    def ensure_supported(self, provisioner_instance: Instance | Any) -> None:
        """
        Ensure the provisioner instance exposes the expected tunnel API.

        Args:
            provisioner_instance: Candidate provisioner instance.
        ...
        """
        tunnel_method = getattr(provisioner_instance, "tunnel", None)
        if callable(tunnel_method):
            return
        msg = "Resolved provisioner instance does not support SSM tunnel creation."
        raise TunnelConfigurationError(msg)


class ProvisionerLocator:
    """
    Find the provisioner instance used for tunnel-backed connections.

    Args:
        settings: Runtime settings that define provisioner tag filters.
    ...
    #: Runtime settings that define provisioner discovery filters.
    ...
    def __init__(
        ...
        *,
        ...
        """
        Initialize the provisioner locator.
    ...
    def find_instance(
        ...
        *,
        ...
        """
        Return the first running provisioner instance in one VPC.
        ...
        filters = [
            Filter(
                Name=f"tag:{self.settings.tunnel.provisioner_tag_key}",
                Values=[self.settings.tunnel.provisioner_tag_value],
            ),
...
class TunnelAwareConnectionResolver:
    ...
    Keyword Args:
        provisioner_locator: Custom provisioner locator for tests or alternate
            lookup behavior.
    ...
    settings: BotocraftSettings
    #: Locator used to find provisioner instances in a target VPC.
    provisioner_locator: ProvisionerLocator
    ...
    def __init__(
        ...
        *,
        settings: BotocraftSettings | None = None,
        provisioner_locator: ProvisionerLocator | Any | None = None,
        ...
        self.settings = settings or BotocraftSettings()
        #: Locator used to find provisioner instances in a target VPC.
        self.provisioner_locator = provisioner_locator or ProvisionerLocator(
    ...
    def open_connection_target(  # noqa: PLR0913
        ...
        *,
        ...
        Keyword Args:
            session: Explicit boto3 session override for provisioner lookup.
            profile: AWS profile forwarded to the provisioner tunnel helper.
            resource_label: Human-friendly resource description used in error
                messages.

        Raises:
            ConnectionResolutionError: No provisioner instance can be found when a
                tunnel is required.
            TunnelConfigurationError: The resolved provisioner instance cannot open
                tunnels.
        ...
        provisioner = self.provisioner_locator.find_instance(vpc_id, session=session)
        if provisioner is None:
            label = resource_label or f"{host}:{port}"
            msg = (
                f"Unable to connect to {label}: no running provisioner instance "
                f"matched tag {self.settings.tunnel.provisioner_tag_key}="
                f"{self.settings.tunnel.provisioner_tag_value!r} in VPC '{vpc_id}'."
            )
            raise ConnectionResolutionError(msg)

        self.capability_validator.ensure_supported(provisioner)
        tunnel_context = provisioner.tunnel(
            host=host,
            remote_port=port,
            profile=profile,
        ...
        return ResolvedConnectionTarget(
            ...
            port=port,
            tunneled=True,
            provisioner_instance=provisioner,
