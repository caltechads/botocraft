...
class QueueManager(QueueManagerMixin, Boto3ModelManager):
    ...
    def list(
        ...
        the specified value are
        returned.

        Keyword Args:
            QueueNamePrefix: A string to use for filtering the list results. Only those
                queues whose name begins with the
    ...
    def purge(self, QueueUrl: str) -> "None":
        ...
        Args:
            QueueUrl: The URL of the queue from which the ``PurgeQueue`` action deletes
                messages.
        """
        args: dict[str, Any] = dict(QueueUrl=self.serialize(QueueUrl))
    ...
    def add_permission(
        ...
        Args:
            ...
            Label: The unique identification of the permission you're setting (for
                example, ``AliceSendMessage``). Maximum 80
            ...
            AWSAccountIds: The Amazon Web Services account numbers of the `principals
                ...
                about locating the Amazon Web Services account identification, see `Your
                Amazon Web Services Identifiers
    ...
    def delete_message(self, QueueUrl: str, ReceiptHandle: str) -> "None":
        ...
        a queue even if a visibility timeout setting causes the message to be locked by
        another consumer. Amazon SQS
        automatically deletes messages left in a queue longer than the retention period
    ...
    def receive_messages(
        ...
        *Amazon SQS Developer Guide*.
        ...
        Keyword Args:
            AttributeNames: This parameter has been discontinued but will be supported
                for backward compatibility. To provide
                attribute names, you are encouraged to use
                ``MessageSystemAttributeNames``.
            MessageSystemAttributeNames: A list of attributes that need to be returned
                along with each message. These attributes
                include:
            MessageAttributeNames: The name of the message attribute, where *N* is the
                index.
            MaxNumberOfMessages: The maximum number of messages to return. Amazon SQS
                never returns more messages than this
            ...
            EventFactoryClass: The class to use to convert the message body to an event
                object. If not provided, the default
        ...
        *Amazon SQS Developer Guide*.
        ...
        Args:
            ...
            ReceiptHandle: The receipt handle associated with the message, whose
                visibility timeout is changed. This parameter
                is returned by the ``ReceiveMessage``  action.
            VisibilityTimeout: The new value for the message's visibility timeout (in
                seconds). Values range: ``0`` to
    ...
    def change_message_visibility_batch(
        ...
        Args:
            QueueUrl: The URL of the Amazon SQS queue whose messages' visibility is
                changed.
            Entries: Lists the receipt handles of the messages for which the visibility
    ...
    def send(
        ...
        *,
        ...
        Args:
            ...
            MessageBody: The message to send. The minimum size is one character. The
                maximum size is 1 MiB or 1,048,576 bytes

        Keyword Args:
            DelaySeconds: The length of time, in seconds, for which to delay a specific
                message. Valid values: 0 to 900.
                Maximum: 15 minutes. Messages with a positive ``DelaySeconds`` value
                    become available for processing after the delay
            ...
            MessageAttributes: Each message attribute consists of a ``Name``, ``Type``,
                ...
                message-metadata.html#sqs- message-attributes>`_ in the *Amazon SQS
                Developer Guide*.
            MessageSystemAttributes: The message system attribute to send. Each message
                system attribute consists of a ``Name``,
                ``Type``, and ``Value``.
            MessageDeduplicationId: This parameter applies only to FIFO
                (first-in-first-out) queues.
            MessageGroupId: ``MessageGroupId`` is an attribute used in Amazon SQS FIFO
                (First-In-First-Out) and standard queues.
                In FIFO queues, ``MessageGroupId`` organizes messages into distinct
                groups. Messages within the same message group
                are always processed one at a time, in strict order, ensuring that no
                two messages from the same group are processed
                simultaneously. In standard queues, using ``MessageGroupId`` enables
                fair queues. It is used to identify the tenant
                a message belongs to, helping maintain consistent message dwell time
                across all tenants during noisy neighbor
                events. Unlike FIFO queues, messages with the same ``MessageGroupId``
                can be processed in parallel, maintaining the
...
class Queue(TagsDictMixin, QueueModelMixin, PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = QueueManager
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`QueueUrl` attribute.
    ...
    def name(self) -> str | None:
        ...
        This is the value of the :py:attr:`QueueName` attribute.
...
class MessageAttributeValue(Boto3Model):
    ...
    For string data types, the ``Value`` attribute has the same restrictions on the
    content as the message body. For more information, see  ``SendMessage.``
...
class Message(MessageModelMixin, PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = MessageManager
...
class ListQueuesResult(Boto3Model):
    ...
    Token value is ``null`` if there are no additional results to request, or if you did
...
class SendMessageBatchRequestEntry(Boto3Model):
    ...
    Valid values: 0 to 900. Maximum: 15 minutes. Messages with a positive
        ``DelaySeconds`` value become available for processing after the delay period is
        finished. If
    ...
    In FIFO queues, ``MessageGroupId`` organizes messages into distinct groups. Messages
    within the same message group are always processed one at a time, in strict order,
    ensuring that no two messages from the same group are processed simultaneously. In
    standard queues, using ``MessageGroupId`` enables fair queues. It is used to
    identify the tenant a message belongs to, helping maintain consistent message dwell
    time across all tenants during noisy neighbor events. Unlike FIFO queues, messages
