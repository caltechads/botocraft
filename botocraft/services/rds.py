...
class DBInstanceManager(Boto3ModelManager):
    ...
    def create(
        ...
        Keyword Args:
            ...
            DBParameterGroupName: The name of the DB parameter group to associate with
                this DB instance. If you don't specify a
                value, then Amazon RDS uses the default DB parameter group for the
                specified DB engine and version.
            ...
            Domain: The Active Directory directory ID to create the DB instance in.
                Currently, you can create only Db2, MySQL,
            ...
            TagSpecifications: Tags to assign to resources associated with the DB
                instance.
            MasterUserAuthenticationType: Specifies the authentication type for the
                master user. With IAM master user
    ...
    def update(
        ...
        can make to your DB instance, call
        ...
        Keyword Args:
            DBSubnetGroupName: The new DB subnet group for the DB instance. You can use
                this parameter to move your DB instance
                to a different VPC. If your DB instance isn't in a VPC, you can also use
                this parameter to move your DB instance
                into a VPC. For more information, see `Working with a DB instance in a
                VPC
            ...
            VpcSecurityGroupIds: A list of Amazon EC2 VPC security groups to associate
                with this DB instance. This change is
            ...
            AllowMajorVersionUpgrade: Specifies whether major version upgrades are
                allowed. Changing this parameter doesn't
            ...
            NewDBInstanceIdentifier: The new identifier for the DB instance when
                renaming a DB instance. When you change the DB
                instance identifier, an instance reboot occurs immediately if you enable
                ``ApplyImmediately``, or will occur during
                the next maintenance window if you disable ``ApplyImmediately``. This
                value is stored as a lowercase string.
            TdeCredentialPassword: The password for the given ARN from the key store in
                order to access the device.
            Domain: The Active Directory directory ID to move the DB instance to.
                Specify ``none`` to remove the instance from
                its current domain. You must create the domain before this operation.
                Currently, you can create only Db2, MySQL,
                Microsoft SQL Server, Oracle, and PostgreSQL DB instances in an Active
                Directory Domain.
            DomainFqdn: The fully qualified domain name (FQDN) of an Active Directory
            ...
            ResumeFullAutomationModeMinutes: The number of minutes to pause the
                automation. When the time period ends, RDS
        ...
        *,
        ...
        Args:
            DBInstanceIdentifier: The DB instance identifier for the DB instance to be
                deleted. This parameter isn't case-
                sensitive.

        Keyword Args:
            SkipFinalSnapshot: Specifies whether to skip the creation of a final DB
                snapshot before deleting the instance. If
                you enable this parameter, RDS doesn't create a DB snapshot. If you
                don't enable this parameter, RDS creates a DB
            ...
            FinalDBSnapshotIdentifier: The ``DBSnapshotIdentifier`` of the new
                ``DBSnapshot`` created when the
                ``SkipFinalSnapshot`` parameter is disabled.
            DeleteAutomatedBackups: Specifies whether to remove automated backups
                immediately after the DB instance is deleted.
                This parameter isn't case- sensitive. The default is to remove automated
                backups immediately after the DB instance
                is deleted.
        """
        ...
        This API supports pagination.
                Args:
                    DBInstanceIdentifier: The user-supplied instance identifier or the
                        Amazon Resource Name (ARN) of the DB instance. If
                        this parameter is specified, information from only the specific
                        DB instance is returned. This parameter isn't case-
                        sensitive.

                Keyword Args:
                    Filters: A filter that specifies one or more DB instances to
                        describe.
        """
        args: dict[str, Any] = dict(
            DBInstanceIdentifier=self.serialize(DBInstanceIdentifier),
...
class RDSDBSubnetGroupManager(Boto3ModelManager):
    ...
    def get(self, DBSubnetGroupName: str) -> "RDSDBSubnetGroup | None":
        """
...
class ProcessorFeature(Boto3Model):
    ...
    ``Name`` parameter. To specify the number
    of threads per core, use the ``threadsPerCore`` feature name for the ``Name``
    parameter.
    ...
    * The current number CPU cores and threads is set to a non-default value.
    ...
    For more information, see  `Configuring the processor for a DB instance class in RDS
    for
...
class RDSPendingModifiedValues(Boto3Model):
    ...
    If ``full``, the DB instance automates monitoring and instance recovery. If ``all-
    paused``, the instance pauses automation for the duration set by ``--resume- full-
    automation-mode-minutes``.
    """
...
class OptionGroupMembership(Boto3Model):
    ...
    removal``, ``pending-maintenance-apply``, ``pending-maintenance-removal``,
...
class DBInstance(TagsDictMixin, RDSInstanceModelMixin, PrimaryBoto3Model):
    """
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = DBInstanceManager
    ...
    The default is ``open-read-only``. For more information, see
    `Working with replicas for Amazon RDS for Db2 <https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/db2-replication.html>`_
    and
    ...
    onRDS/latest/AuroraUserGuide/Concepts.AuroraHighAvailability.html#Aurora.Managing.FaultTolerance>`_
    in the *Amazon Aurora
    User Guide*.
    """
    ...
    for RDS for Db2 and RDS for SQL Server DB instances that were created with a time
    zone specified.
    """
    ...
    To migrate to the preferred configuration, you can either create a blue/green
    deployment, or create a read replica from the DB
    ...
    def subnets(self) -> "list[Subnet] | None":
        """
        Return the EC2 subnets associated with this DB instance, if any.
...
class RDSDBSubnetGroup(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = RDSDBSubnetGroupManager
...
class ModifyAdditionalStorageVolume(Boto3Model):
    ...
    This setting is only supported for Provisioned IOPS SSD (``io1`` and ``io2``)
    storage types.
    """
    ...
    The value ``true`` schedules the volume for deletion. You can delete an additional
    storage volume only when it doesn't contain database files or other data.
    """
