...
class ToolInputSchema(Boto3Model):
    ...
    Converse API <https://docs.aws.amazon.com/bedrock/latest/userguide/tool-use.html>`_ in the Amazon Bedrock User Guide.
    """
...
class AutoToolChoice(Boto3Model):
    ...
    API <https://docs.aws.amazon.com/bedrock/latest/userguide/tool-use.html>`_ in the Amazon Bedrock User Guide
    """
...
class AnyToolChoice(Boto3Model):
    ...
    For example, ``{"any" : {}}``. For more information,
    see `Call a tool with the Converse API <https://docs.aws.amazon.com/bedrock/latest/userguide/tool-use.html>`_ in the
    Amazon Bedrock User Guide.
    """
...
class SpecificToolChoice(Boto3Model):
    """The model must request a specific tool. For example, ``{"tool" : {"name" : "Your tool name"}}``. For more information,
    see `Call a tool with the Converse API <https://docs.aws.amazon.com/bedrock/latest/userguide/tool-use.html>`_ in the
    Amazon Bedrock User Guide
...
class CountTokensInput(Boto3Model):
    ...
    Use this field when you want to count tokens for a raw text input
    that would be sent to the ``InvokeModel`` operation.
    """
    converse: "ConverseTokensRequest | None" = None
    """
    A ``Converse`` request for which to count tokens.

    Use this field when you want to count tokens for a conversation-based
    input that would be sent to the ``Converse`` operation.
    """
