...
class SecurityGroupManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def revoke_egress(
        ...
        *,
        ...
        Args:
            GroupId: The ID of the security group.
            IpPermissions: The sets of IP permissions. You can't specify a destination security group and a CIDR IP address
...
class NetworkAclManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def create_entry(
        ...
        *,
        ...
        Args:
            NetworkAclId: The ID of the network ACL.
            RuleNumber: The rule number for the entry (for example, 100). ACL entries are processed in ascending order by rule
                number.
            Protocol: The protocol number. A value of "-1" means all protocols. If you specify "-1" or a protocol number other
                than "6" (TCP), "17" (UDP), or "1" (ICMP), traffic on all ports is allowed, regardless of any ports or ICMP types or
                codes that you specify. If you specify protocol "58" (ICMPv6) and specify an IPv4 CIDR block, traffic for all ICMP
        ...
        Keyword Args:
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            CidrBlock: The IPv4 network range to allow or deny, in CIDR notation (for example ``172.16.0.0/24``). We modify the
                specified CIDR block to its canonical form; for example, if you specify ``100.68.0.18/18``, we modify it to
                ``100.68.0.0/18``.
            Ipv6CidrBlock: The IPv6 network range to allow or deny, in CIDR notation (for example ``2001:db8:1234:1a00::/64``).
            IcmpTypeCode: ICMP protocol: The ICMP or ICMPv6 type and code. Required if specifying protocol 1 (ICMP) or protocol
                58 (ICMPv6) with an IPv6 CIDR block.
            PortRange: TCP or UDP protocols: The range of ports the rule applies to. Required if specifying protocol 6 (TCP) or
                17 (UDP).
        """
        args: dict[str, Any] = dict(
    ...
    def replace_entry(
        ...
        *,
        ...
        ACLs <https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html>`_ in the *Amazon VPC User Guide*.
...
class InstanceManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def get(
        ...
        *,
        ...
        Keyword Args:
            IncludeManagedResources: Indicates whether to include managed resources in the output. If this parameter is set to
                ``true``, the output includes resources that are managed by Amazon Web Services services, even if managed resource
    ...
    def modify_instance_type(
        ...
        *,
        ...
        Args:
            ...
            InstanceType: Changes the instance type to the specified value. For more information, see `Instance types
        ...
        Keyword Args:
            SourceDestCheck: Enable or disable source/destination checks, which ensure that the instance is either the source or
                the destination of any traffic that it receives. If the value is ``true``, source/destination checks are enabled;
                otherwise, they are disabled. The default value is ``true``. You must disable source/destination checks if the
                instance runs services such as network address translation, routing, or firewalls.
            EnclaveOptions: Enables or disables the instance for Amazon Web Services Nitro Enclaves. For more information, see
            ...
            BlockDeviceMappings: Modifies the ``DeleteOnTermination`` attribute for volumes that are currently attached. The
                volume must be owned by the caller. If no value is specified for ``DeleteOnTermination``, the default is ``true``
                and the volume is deleted when the instance is terminated. You can't modify the ``DeleteOnTermination`` attribute
                for volumes that are attached to Amazon Web Services-managed resources.
            DisableApiTermination: Enable or disable termination protection for the instance. If the value is ``true``, you
                can't terminate the instance using the Amazon EC2 console, command line interface, or API. You can't enable
            ...
            Ramdisk: Changes the instance's RAM disk to the specified value. We recommend that you use PV-GRUB instead of
                ...
                <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/UserProvidedKernels.html>`_.
            UserData: Changes the instance's user data to the specified value. User data must be base64-encoded. Depending on
                the tool or SDK that you're using, the base64-encoding might be performed for you. For more information, see `Work
            ...
            InstanceInitiatedShutdownBehavior: Specifies whether an instance stops or terminates when you initiate shutdown from
                the instance (using the operating system command for system shutdown).
            Groups: Replaces the security groups of the instance with the specified security groups. You must specify the ID of
                at least one security group, even if it's just the default security group for the VPC.
            EbsOptimized: Specifies whether the instance is optimized for Amazon EBS I/O. This optimization provides dedicated
                ...
                instance.
            SriovNetSupport: Set to ``simple`` to enable enhanced networking with the Intel 82599 Virtual Function interface for
                the instance.
...
class NetworkInterfaceManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def assign_private_ips(
        ...
        *,
        ...
        Keyword Args:
            Ipv4Prefixes: One or more IPv4 prefixes assigned to the network interface. You can't use this option if you use the
                ``Ipv4PrefixCount`` option.
            Ipv4PrefixCount: The number of IPv4 prefixes that Amazon Web Services automatically assigns to the network
                interface. You can't use this option if you use the ``Ipv4 Prefixes`` option.
            PrivateIpAddresses: The IP addresses to be assigned as a secondary private IP address to the network interface. You
                can't specify this parameter when also specifying a number of secondary IP addresses.
            SecondaryPrivateIpAddressCount: The number of secondary IP addresses to assign to the network interface. You can't
                specify this parameter when also specifying private IP addresses.
            AllowReassignment: Indicates whether to allow an IP address that is already assigned to another network interface or
    ...
    def unassign_private_ips(
        ...
        *,
        ...
        Keyword Args:
            ...
            PrivateIpAddresses: The secondary private IP addresses to unassign from the network interface. You can specify this
        ...
        *,
        ...
        Keyword Args:
            EnaSrdSpecification: Updates the ENA Express configuration for the network interface that’s attached to the
                instance.
            EnablePrimaryIpv6: If you’re modifying a network interface in a dual-stack or IPv6-only subnet, you have the option
                to assign a primary IPv6 IP address. A primary IPv6 address is an IPv6 GUA address associated with an ENI that you
                have enabled to use a primary IPv6 address. Use this option if the instance that this ENI will be attached to relies
                on its IPv6 address not changing. Amazon Web Services will automatically assign an IPv6 address associated with the
                ENI attached to your instance to be the primary IPv6 address. Once you enable an IPv6 GUA address to be a primary
            ...
            AssociatePublicIpAddress: Indicates whether to assign a public IPv4 address to a network interface. This option can
                be enabled for any network interface but will only apply to the primary network interface (eth0).
            AssociatedSubnetIds: A list of subnet IDs to associate with the network interface.
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            ...
            Groups: Changes the security groups for the network interface. The new set of groups you specify replaces the
                current set. You must specify at least one group, even if it's just the default security group in the VPC. You must
    ...
    def get(
        ...
        *,
        ...
        Keyword Args:
            ...
            IncludeUnsupportedInRegion: If ``true``, the response includes instance types that are not supported in the current
...
class PlacementGroupManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def delete(self, GroupName: str, *, DryRun: bool = False) -> None:
        ...
        placement group. For more information, see `Placement
...
class DhcpOptionsManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def get(self, DhcpOptionsId: str, *, DryRun: bool = False) -> "DhcpOptions | None":
        ...
        sets. Alternatively, you can specify specific DHCP option set IDs or filter the
        results to include only the DHCP option sets that match specific criteria.
...
class VolumeManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def update(
        ...
        stopping the instance or detaching the volume from it. For more information about modifying EBS volumes, see `Amazon EBS
    ...
    def attach(
        ...
        *,
        ...
        Keyword Args:
            EbsCardIndex: The index of the EBS card. Some instance types support multiple EBS cards. The default EBS card index
                is 0.
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
        ...
        *,
        ...
        Keyword Args:
            ...
            Force: Forces detachment if the previous detachment attempt did not occur cleanly (for example, logging into an
                ...
                system. Use this option only as a last resort to detach a volume from a failed instance. The instance won't have an
                opportunity to flush file system caches or file system metadata. If you use this option, you must perform file
                system check and repair procedures.
            InstanceId: The ID of the instance. If you are detaching a Multi-Attach enabled volume, you must specify an instance
                ID.
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
...
class RouteTableManager(EC2TagsManagerMixin, Boto3ModelManager):
        ...
        *,
        ...
        Keyword Args:
            ...
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            DestinationCidrBlock: The IPv4 CIDR address block used for the destination match. Routing decisions are based on the
                most specific match. We modify the specified CIDR block to its canonical form; for example, if you specify
            ...
            DestinationIpv6CidrBlock: The IPv6 CIDR block used for the destination match. Routing decisions are based on the
                most specific match.
            EgressOnlyInternetGatewayId: [IPv6 traffic only] The ID of an egress-only internet gateway.
            InstanceId: The ID of a NAT instance in your VPC. The operation fails if you specify an instance ID unless exactly
        ...
        *,
        ...
        Keyword Args:
            ...
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            DestinationCidrBlock: The IPv4 CIDR range for the route. The value you specify must match the CIDR for the route
                exactly.
            DestinationIpv6CidrBlock: The IPv6 CIDR range for the route. The value you specify must match the CIDR for the route
                exactly.
        """
        args: dict[str, Any] = dict(
            RouteTableId=self.serialize(RouteTableId),
        ...
        *,
        ...
        Keyword Args:
            ...
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            DestinationCidrBlock: The IPv4 CIDR address block used for the destination match. The value that you provide must
                match the CIDR of an existing route in the table.
            GatewayId: The ID of an internet gateway or virtual private gateway.
            DestinationIpv6CidrBlock: The IPv6 CIDR address block used for the destination match. The value that you provide
    ...
    def associate(
        ...
        *,
        ...
        Keyword Args:
            ...
            PublicIpv4Pool: The ID of a public IPv4 pool. A public IPv4 pool is a pool of IPv4 addresses that you've brought to
    ...
    def disassociate(self, AssociationId: str, *, DryRun: bool = False) -> None:
        ...
        Args:
            AssociationId: The association ID representing the current association between the route table and subnet or
                gateway.
...
class NatGatewayManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def create(
        ...
        Keyword Args:
            ClientToken: Unique, case-sensitive identifier that you provide to ensure the idempotency of the request. For more
                information, see `Ensuring idempotency <https://docs.aws.amazon.com/ec2/latest/devguide/ec2-api-idempotency.html>`_.
            AvailabilityZoneAddresses: For regional NAT gateways only: Specifies which Availability Zones you want the NAT
                gateway to support and the Elastic IP addresses (EIPs) to use in each AZ. The regional NAT gateway uses these EIPs
                to handle outbound NAT traffic from their respective AZs. If not specified, the NAT gateway will automatically
                expand to new AZs and associate EIPs upon detection of an elastic network interface. If you specify this parameter,
                auto-expansion is disabled and you must manually manage AZ coverage.
            PrivateIpAddress: The private IPv4 address to assign to the NAT gateway. If you don't provide an address, a private
                IPv4 address will be automatically assigned.
            SecondaryAllocationIds: Secondary EIP allocation IDs. For more information, see `Create a NAT gateway
                <https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-working-with.html>`_ in the *Amazon VPC User Guide*.
            SecondaryPrivateIpAddresses: Secondary private IPv4 addresses. For more information about secondary addresses, see
                `Create a NAT gateway <https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-working-with.html>`_ in the
                *Amazon VPC User Guide*.
            SecondaryPrivateIpAddressCount: `Private NAT gateway only] The number of secondary private IPv4 addresses you want
                to assign to the NAT gateway. For more information about secondary addresses, see [Create a NAT gateway
                <https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-working-with.html>`_ in the *Amazon VPC User Guide*.
        """
        data = model.model_dump(exclude_none=True, by_alias=True)
        args = dict(
    ...
    def associate_address(
        ...
        *,
        ...
        """
        Associates Elastic IP addresses (EIPs) and private IPv4 addresses with a public NAT gateway. For more information, see
        `Work with NAT gateways <https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-working-with.html>`_ in the *Amazon
        VPC User Guide*.
        ...
        Keyword Args:
            ...
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            AvailabilityZone: For regional NAT gateways only: The Availability Zone where you want to associate an Elastic IP
                address (EIP). The regional NAT gateway uses a separate EIP in each AZ to handle outbound NAT traffic from that AZ.
            AvailabilityZoneId: For regional NAT gateways only: The ID of the Availability Zone where you want to associate an
                Elastic IP address (EIP). The regional NAT gateway uses a separate EIP in each AZ to handle outbound NAT traffic
                from that AZ. Use this instead of AvailabilityZone for consistent identification of AZs across Amazon Web Services
                Regions.
    ...
    def disassociate_address(
        ...
        *,
        ...
        """
        Disassociates secondary Elastic IP addresses (EIPs) from a public NAT gateway. You cannot disassociate your primary EIP.
        For more information, see `Edit secondary IP address associations <https://docs.aws.amazon.com/vpc/latest/userguide/nat-
        gateway-working-with.html#nat-gateway-edit-secondary>`_ in the *Amazon VPC User Guide*.
        ...
        Keyword Args:
            MaxDrainDurationSeconds: The maximum amount of time to wait (in seconds) before forcibly releasing the IP addresses
...
class VpcPeeringConnectionManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def create(self, model: "VpcPeeringConnection") -> "VpcPeeringConnection":
        ...
        Region to the requester VPC. The requester VPC and accepter VPC cannot have overlapping CIDR blocks.
...
class EC2VpcEndpointManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def create(
        ...
        endpoint service. You can use an endpoint service provided by Amazon Web Services, an Amazon Web Services Marketplace
        Partner, or another Amazon Web Services account. For more information, see the `Amazon Web Services PrivateLink User
        ...
        Keyword Args:
            SecurityGroupIds: (Interface endpoint) The IDs of the security groups to associate with the endpoint network
    ...
    def modify(
        ...
        *,
        ...
        (interface, gateway, or Gateway Load Balancer). For more information, see the `Amazon Web Services PrivateLink
        Guide <https://docs.aws.amazon.com/vpc/latest/privatelink/>`_.
        ...
        Keyword Args:
            DryRun: Checks whether you have the required permissions for the action, without actually making the request, and
                provides an error response. If you have the required permissions, the error response is ``DryRunOperation``.
                Otherwise, it is ``UnauthorizedOperation``.
            ResetPolicy: (Gateway endpoint) Specify ``true`` to reset the policy document to the default policy. The default
            ...
            AddSecurityGroupIds: (Interface endpoint) The IDs of the security groups to associate with the endpoint network
                interfaces.
            RemoveSecurityGroupIds: (Interface endpoint) The IDs of the security groups to disassociate from the endpoint
                network interfaces.
...
class VpnConnectionManager(EC2TagsManagerMixin, Boto3ModelManager):
    ...
    def create(
        ...
        Keyword Args:
            PreSharedKeyStorage: Specifies the storage mode for the pre-shared key (PSK). Valid values are ``Standard``" (stored
                in the Site-to-Site VPN service) or ``SecretsManager`` (stored in Amazon Web Services Secrets Manager).
        """
        data = model.model_dump(exclude_none=True, by_alias=True)
        args = dict(
...
class NetworkCardInfo(Boto3Model):
    ...
    This number is in addition to the base number specified by ``maximumNetworkInterfaces``.
    """
...
* For RSA key pairs, the key fingerprint is the SHA-1 digest of the DER encoded private key.
    ...
    CreateTime: datetime = Field(default=None, frozen=True)
    """
If you used Amazon EC2 to create the key pair, this is the date and time when the key was created, in `ISO 8601 date-
time format <https://www.iso.org/iso-8601-date-and-time-format.html>`_, in the UTC time zone.
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`KeyName`
        attribute.
...
class PlacementGroup(TagsDictMixin, PrimaryBoto3Model):
    ...
    *Only* Outpost placement groups can be spread across hosts.
    ...
    def arn(self) -> str | None:
        """
        Return the ARN of the model.   This is the value of the :py:attr:`GroupArn`
        attribute.
...
class CustomerGateway(TagsDictMixin, PrimaryBoto3Model):
    ...
    private IPv4 address. If ``OutsideIpAddressType`` is set to ``PublicIpv4``, you can use a public IPv4 address. If
    ``OutsideIpAddressType`` is set to ``Ipv6``, you can use a public IPv6 address.
    """
...
class VpnGateway(TagsDictMixin, PrimaryBoto3Model):
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`VpnGatewayId`
        attribute.
...
class Volume(TagsDictMixin, PrimaryBoto3Model):
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`VolumeId`
        attribute.
...
class Route(Boto3Model):
    ...
    specified gateway isn't attached to the VPC, or the specified NAT instance has been terminated).
    """
    VpcPeeringConnectionId: "str | None" = None
    """
...
class RouteTable(TagsDictMixin, PrimaryBoto3Model):
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`RouteTableId`
        attribute.
...
class NatGateway(TagsDictMixin, PrimaryBoto3Model):
    ...
    """
For regional NAT gateways only: Indicates whether Amazon Web Services automatically allocates additional Elastic IP
addresses (EIPs) in an AZ when the NAT gateway needs more ports due to increased concurrent connections to a single
destination from that AZ.
    ...
    """
For regional NAT gateways only: Indicates whether Amazon Web Services automatically manages AZ coverage. When enabled,
the NAT gateway associates EIPs in all AZs where your VPC has subnets to handle outbound NAT traffic, expands to new AZs
when you create subnets there, and retracts from AZs where you've removed all subnets. When disabled, you must manually
manage which AZs the NAT gateway supports and their corresponding EIPs.
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`NatGatewayId`
        attribute.
...
class EC2DnsOptions(Boto3Model):
    ...
    Only
    supported when private DNS is enabled and the private DNS preference is ``VERIFIED_DOMAINS_AND_SPECIFIED_DOMAINS`` or
...
class VpnConnectionOptions(Boto3Model):
    ...
    Represents the current throughput capacity setting for the tunnel
    connection. ``standard`` tunnel bandwidth supports up to 1.25 Gbps per tunnel while ``large`` supports up to 5 Gbps per
    tunnel. If no tunnel bandwidth was specified for the connection, ``standard`` is used as the default value.
    """
...
class VpnConnection(TagsDictMixin, PrimaryBoto3Model):
    ...
    This element is
    always present in the CreateVpnConnection response; however, it's present in the DescribeVpnConnections response only if
...
class FlowLog(TagsDictMixin, PrimaryBoto3Model):
    ...
    ``Rate limited`` indicates that CloudWatch Logs throttling has been applied
    for one or more network interfaces, or that you've reached the limit on the number of log groups that you can create.
    ``Access error`` indicates that the IAM role associated with the flow log does not have sufficient permissions to
    ...
    def name(self) -> str | None:
        """
        Return the name of the model.   This is the value of the :py:attr:`FlowLogId`
        attribute.
...
class AvailabilityZoneAddress(Boto3Model):
    """For regional NAT gateways only: The configuration specifying which Elastic IP address (EIP) to use for handling outbound
    NAT traffic from a specific Availability Zone.
    ...
    AvailabilityZone: "str | None" = None
    """
For regional NAT gateways only: The Availability Zone where this specific NAT gateway configuration will be active. Each
AZ in a regional NAT gateway has its own configuration to handle outbound NAT traffic from that AZ.
    """
    AvailabilityZoneId: "str | None" = None
    """
For regional NAT gateways only: The ID of the Availability Zone where this specific NAT gateway configuration will be
active. Each AZ in a regional NAT gateway has its own configuration to handle outbound NAT traffic from that AZ. Use
this instead of AvailabilityZone for consistent identification of AZs across Amazon Web Services Regions.
    """
...
class VpnTunnelOptionsSpecification(Boto3Model):
    ...
    Specify ``restart`` to restart the IKE initiation. Specify ``clear`` to end
    the IKE session.
    """
    ...
    By default, your customer gateway device
    must initiate the IKE negotiation and bring up the tunnel. Specify ``start`` for Amazon Web Services to initiate the IKE
    negotiation.
    """
...
class VpnConnectionOptionsSpecification(Boto3Model):
    ...
    Gbps per tunnel. The default value is ``standard``. Existing VPN connections without a bandwidth setting will
    automatically default to ``standard``.
    """
    StaticRoutesOnly: "bool | None" = None
    """
    ...
    not support BGP, you must specify ``true``. Use CreateVpnConnectionRoute to create a static route.
    """
...
class DestinationOptionsRequest(Boto3Model):
    ...
    This reduces the cost and response time for queries. The default
    is ``false``.
    """
