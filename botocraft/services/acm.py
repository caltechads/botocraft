...
class ACMCertificateManager(Boto3ModelManager):
    ...
    def import_certificate(
        ...
        *,
        ...
        Note that `integrated services <https://docs.aws.amazon.com/acm/latest/userguide/acm-services.html>`_ allow only certificate types
        and keys they support to be associated with their resources. Further, their
        support differs depending on whether the
        certificate is imported into IAM or into ACM. For more information, see the
        documentation for each service. For more
        ...
        Keyword Args:
            CertificateArn: The `Amazon Resource Name (ARN) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-
                namespaces.html>`_ of an imported certificate to replace. To import a
    ...
    def delete(self, CertificateArn: str) -> None:
        ...
        consistent. The may be a short
        delay before the certificate no longer appears in the list that can be displayed
        by calling the ListCertificates action
        or be retrieved by calling the GetCertificate action.

        Args:
            CertificateArn: String that contains the ARN of the ACM certificate to be
                deleted. This must be of the form:
        """
    ...
    def get(self, CertificateArn: str) -> "ACMCertificate | None":
        ...
        Args:
            CertificateArn: The Amazon Resource Name (ARN) of the ACM certificate. The
        ...
        *,
        ...
        Keyword Args:
            ...
            Includes: Filter the certificate list. For more information, see the Filters
                structure.
            MaxItems: Use this parameter when paginating results to specify the maximum
                number of items to return in the
                response. If additional items exist beyond the number you specify, the
                ``NextToken`` element is sent in the
                response. Use this ``NextToken`` value in a subsequent request to
                retrieve additional items.
            SortBy: Specifies the field to sort results by. If you specify ``SortBy``,
                you must also specify ``SortOrder``.
            SortOrder: Specifies the order of sorted results. If you specify
                ``SortOrder``, you must also specify ``SortBy``.
        """
    ...
    def renew(self, CertificateArn: str) -> "None":
        ...
        renew your Amazon Web Services Private CA certificates with ACM, you must first
        `grant the ACM service principal
        ...
        Args:
            CertificateArn: String that contains the ARN of the ACM certificate to be
                renewed. This must be of the form:
    ...
    def request(
        ...
        *,
        ...
        Args:
            DomainName: Fully qualified domain name (FQDN), such as www.example.com,
                that you want to secure with an ACM
                certificate. Use an asterisk (*) to create a wildcard certificate that
                protects several sites in the same domain.
        ...
        Keyword Args:
            ValidationMethod: The method you want to use if you are requesting a public
                certificate to validate that you own or
            ...
            SubjectAlternativeNames: Additional FQDNs to be included in the Subject
                Alternative Name extension of the ACM
                certificate. For example, add the name www.example.net to a certificate
                for which the ``DomainName`` field is
                www.example.com if users can reach your site by using either name. The
                maximum number of domain names that you can
                add to an ACM certificate is 100. However, the initial quota is 10
                domain names. If you need more than 10 names, you
            ...
            IdempotencyToken: Customer chosen string that can be used to distinguish
                between calls to ``RequestCertificate``.
                Idempotency tokens time out after one hour. Therefore, if you call
                ``RequestCertificate`` multiple times with the
                same idempotency token within one hour, ACM recognizes that you are
                requesting only one certificate and will issue
                only one. If you change the idempotency token for each call, ACM
                recognizes that you are requesting multiple
                certificates.
            DomainValidationOptions: The domain name that you want ACM to use to send
                you emails so that you can validate domain
                ownership.
            Options: You can use this parameter to specify whether to add the
                certificate to a certificate transparency log and
                export your certificate.
            CertificateAuthorityArn: The Amazon Resource Name (ARN) of the private
                certificate authority (CA) that will be used
                to issue the certificate. If you do not provide an ARN and you are
                trying to request a private certificate, ACM will
                attempt to issue a public certificate. For more information about
                private CAs, see the `Amazon Web Services Private
            ...
            KeyAlgorithm: Specifies the algorithm of the public and private key pair
                that your certificate uses to encrypt data.
                RSA is the default key algorithm for ACM certificates. Elliptic Curve
                Digital Signature Algorithm (ECDSA) keys are
                smaller, offering security comparable to RSA keys but with greater
                computing efficiency. However, ECDSA is not
                supported by all network clients. Some Amazon Web Services services may
                require RSA keys, or only support ECDSA keys
                of a particular size, while others allow the use of either RSA and ECDSA
                keys to ensure that compatibility is not
                broken. Check the requirements for the Amazon Web Services service where
                you plan to deploy your certificate. For
            ...
            ManagedBy: Identifies the Amazon Web Services service that manages the
    ...
    def revoke(
        ...
        You can only revoke certificates that have been previously exported.
                Args:
                    CertificateArn: The Amazon Resource Name (ARN) of the public or
                        private certificate that will be revoked. The ARN
                        must have the following form:
...
class ACMHttpRedirect(Boto3Model):
    ...
    This field exists only when the certificate type is ``AMAZON_ISSUED`` and the
...
class ACMRenewalSummary(Boto3Model):
    ...
    renewal.html>`_ for the certificate. This structure exists only when the certificate
...
validation that occurs as a result of the RequestCertificate request. This field exists
only when the certificate type
is ``AMAZON_ISSUED``.
    """
...
class CertificateOptions(Boto3Model):
    ...
    You can use this structure to specify whether to opt in to or out of certificate
...
class ACMCertificate(TagsDictMixin, PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = ACMCertificateManager
    ...
    For more information about ARNs, see `Amazon Resource Names (ARNs) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html>`_ in the *Amazon Web Services General
    Reference*.
    """
    ...
    information, see
    `Certificate Request Failed <https://docs.aws.amazon.com/acm/latest/userguide/troubleshooting.html#troubleshooting-failed>`_
    in the
    *Certificate Manager User Guide*.
    ...
    For certificates provided by ACM, this value is ``AMAZON_ISSUED``. For certificates
    that you imported with ImportCertificate, this value is ``IMPORTED``. ACM does not
    provide
    `managed renewal <https://docs.aws.amazon.com/acm/latest/userguide/acm-renewal.html>`_
    for imported certificates. For more    information about the differences between
    certificates that you import and those that ACM provides, see
    `Importing Certificates <https://docs.aws.amazon.com/acm/latest/userguide/import-certificate.html>`_
    ...
    """
Contains information about the status of ACM's `managed renewal <https://docs.aws.amazon.com/acm/latest/userguide/acm-
renewal.html>`_ for the certificate. This field exists only when the certificate type is
``AMAZON_ISSUED``.
    """
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`CertificateArn` attribute.
...
response. If false, the response
includes all of the subject alternative names included in the certificate. If true, the
response only includes the first
100 subject alternative names included in the certificate. To display the full list of
subject alternative names, use
