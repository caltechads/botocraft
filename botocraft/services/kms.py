...
class KMSKeyManager(Boto3ModelManager):
    ...
    def create(
        ...
        encryption and signing. Some Amazon Web Services services let you use KMS keys
        that you create and manage to protect
        ...
        Keyword Args:
            ...
            Tags: Assigns one or more tags to the KMS key. Use this parameter to tag the
                KMS key when it is created. To tag an
            ...
            XksKeyId: Identifies the `external key <https://docs.aws.amazon.com/kms/latest/developerguide/keystore-
                external.html#concept- external-key>`_ that serves as key material for
                the KMS key in an `external key store
    ...
    def get(
        ...
        You can run ``DescribeKey`` on a `customer managed key <https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#customer-mgn-key>`_ or an `Amazon Web Services
    ...
    def list(self, *, Limit: "int | None" = None) -> PrimaryBoto3ModelQuerySet:
        ...
        Keyword Args:
            Limit: Use this parameter to specify the maximum number of items to return.
                When this value is present, KMS does not
    ...
    def delete(
        ...
        the KMS key changes to ``PendingDeletion`` and
        ...
        deletion of the KMS key. After the waiting
        period ends, KMS deletes the KMS key, its key material, and all KMS data
        associated with it, including all aliases that
        refer to it.
        ...
        Keyword Args:
            PendingWindowInDays: The waiting period, specified in number of days. After
                the waiting period ends, KMS deletes the
                KMS key.
        """
        args: dict[str, Any] = dict(
    ...
    def enable(self, KeyId: str) -> None:
        ...
        This allows you to use the KMS key for `cryptographic operations <https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations>`_.
    ...
    def disable(self, KeyId: str) -> None:
        ...
        This change temporarily prevents use of the KMS key for `cryptographic operations <https://docs.aws.amazon.com/kms/latest/developerguide/kms-cryptography.html#cryptographic-operations>`_.
    ...
    def update_alias(self, AliasName: str, TargetKeyId: str) -> "None":
        ...
        Args:
            AliasName: Identifies the alias that is changing its KMS key. This value
                must begin with ``alias/`` followed by the
                alias name, such as ``alias/ExampleAlias``. You cannot use
            ...
            TargetKeyId: Identifies the `customer managed key
                ...
                alias. You don't have permission to associate an alias with an `Amazon
...
class KMSMultiRegionConfiguration(Boto3Model):
    ...
    This field appears only when the KMS key is a primary or replica of a multi-Region
    key.
...
class XksKeyConfigurationType(Boto3Model):
    ...
    external.html#concept-external-key>`_ that is associated with a KMS key in an
    external key store.
...
class KMSKey(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = KMSKeyManager
...
cryptography.html#cryptographic-operations>`_ for which you can use the KMS key.
    ...
    For examples, see `Key Management Service (KMS) <https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html#arn-syntax-kms>`_ in the Example ARNs
    ...
    This value is present only when the KMS key is scheduled for deletion, that is, when
    ...
    When a key material expires, KMS deletes the key material and the KMS key becomes
    unusable. This value is present only for KMS keys whose ``Origin`` is ``EXTERNAL``
    and the ``ExpirationModel`` is ``KEY_MATERIAL_EXPIRES``, otherwise this value is
    omitted.
    """
...
overview.html>`_ that contains the KMS key. This field is present only when the KMS key
is created in a custom key store.
    ...
    KMS creates
    the key material for the KMS key in the associated CloudHSM cluster. This field is
    present only when the KMS key is
    ...
    This value is present only when ``Origin`` is ``EXTERNAL``, otherwise this value is
    omitted.
    """
    ...
    This waiting period begins when the last of its replica keys is deleted. This value
    is present only when the ``KeyState`` of the KMS key is ``PendingReplicaDeletion``.
    That indicates that the KMS key is the primary key in a multi-Region key, it is
    ...
    This value is present for symmetric encryption keys with ``AWS_KMS`` or ``EXTERNAL``
    origin. These KMS keys support automatic or on-demand key rotation and can have
    multiple key materials associated with them. KMS uses the current key material for
    both encryption and decryption, and the non-current key material for decryption
    operations only.
    """
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`KeyId` attribute.
...
class ListKeysResponse(Boto3Model):
    ...
    When this value is true, the list in this response is truncated. To get more items,
