...
class ParameterManager(Boto3ModelManager):
    ...
    def create(
        ...
        Keyword Args:
            ...
            KeyId: The Key Management Service (KMS) ID that you want to use to encrypt a
                parameter. Use a custom key for better
                security. Required for parameters that use the ``SecureString`` data
                type.
            AllowedPattern: A regular expression used to validate the parameter value.
                For example, for String types with values
            ...
            Tags: Optional metadata that you assign to a resource. Tags enable you to
                ...
                such as by purpose, owner, or environment. For example, you might want
                to tag a Systems Manager parameter to
                identify the type of resource to which it applies, the environment, or
                the type of configuration data referenced by
                the parameter. In this case, you could specify the following key-value
                pairs:
            Tier: The parameter tier to assign to a parameter.
            Policies: One or more policies to apply to a parameter. This operation takes
                a JSON array. Parameter Store, a tool
                in Amazon Web Services Systems Manager supports the following policy
                types:
        """
    ...
    def get(self, Name: str, *, WithDecryption: bool = True) -> "Parameter | None":
        """
        ...
        Keyword Args:
            WithDecryption: Return decrypted secure string value. Return decrypted
                values for secure string parameters. This
    ...
    def get_many(
        ...
        Args:
            Names: The names or Amazon Resource Names (ARNs) of the parameters that you
                want to query. For parameters shared
...
class Parameter(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = ParameterManager
    ...
    Valid values include the following: ``String``, ``StringList``, and
        ``SecureString``.
    """
...
class ParameterStringFilter(Boto3Model):
    ...
    The ``Name`` filter additionally supports the ``Contains`` option. (Exception: For
    filters using the key ``Path``, valid options include ``Recursive`` and
    ``OneLevel``.)
    """
...
class ParameterInlinePolicy(Boto3Model):
    ...
    Policies report the following statuses: Pending (the policy hasn't been enforced or
        applied yet), Finished (the policy was applied), Failed (the policy wasn't
        applied), or InProgress (the policy is being applied
    now).
    """
...
class ParameterMetadata(Boto3Model):
    ...
    Valid parameter types include the following: ``String``, ``StringList``, and
        ``SecureString``.
    """
    KeyId: "str | None" = None
    """
    ...
    The parameter tier.
    """
    Policies: "builtins.list[ParameterInlinePolicy] | None" = Field(
