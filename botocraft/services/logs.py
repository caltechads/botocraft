...
class LogGroupManager(LogGroupManagerMixin, Boto3ModelManager):
    ...
    def get(
        ...
        *,
        accountIdentifiers: "builtins.list[str] | None" = None,
        ...
        Args:
            logGroupIdentifier: The identifier of the log group to get. If this is in
                the same account you are querying, you can
                use the log group name, if you also set ````includeLinkedAccounts```` to
                ````true````, you must use the log group
                ARN.

        Keyword Args:
            accountIdentifiers: When ``includeLinkedAccounts`` is set to ``true``, use
                this parameter to specify the list of
                accounts to search. You can specify as many as 20 account IDs in the
                array.
            logGroupNamePattern: If you specify a string for this parameter, the
                operation returns only log groups that have
                names that match the string based on a case-sensitive substring search.
                For example, if you specify ``DataLogs``,
                log groups named ``DataLogs``, ``aws/DataLogs``, and ``GroupDataLogs``
                would match, but ``datalogs``, ``Data/log/s``
                and ``Groupdata`` would not match.
            limit: The maximum number of items returned. If you don't specify a value,
    ...
    def list(
        ...
        *,
        ...
        logGroupIdentifiers: "builtins.list[str] | None" = None,
        ...
        Keyword Args:
            ...
            includeLinkedAccounts: If you are using a monitoring account, set this to
                ``true`` to have the operation return log
                groups in the accounts listed in ``accountIdentifiers``.
            logGroupClass: Use this parameter to limit the results to only those log
                groups in the specified log group class. If
                you omit this parameter, log groups of all classes can be returned.
            logGroupIdentifiers: Use this array to filter the list of log groups
                returned. If you specify this parameter, the
                only other filter that you can choose to specify is
        ...
        *,
        ...
        Keyword Args:
            ...
            time: The time to set as the center of the query. If you specify ``time``,
                the 8 minutes before and 8 minutes after
                this time are searched. If you omit ``time``, the most recent 15 minutes
                up to the current time are searched.
            logGroupIdentifier: Specify either the name or ARN of the log group to view.
                If the log group is in a source account
        ...
        results: "builtins.list[LogGroupField]" = None
        if response is not None:
            results = response.logGroupFields

        self.sessionize(results)
        return cast("builtins.list[LogGroupField]", results)
...
class LogGroup(TagsDictMixin, PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = LogGroupManager

    Arn: str = Field(default=None, alias="arn")
    """
    The Amazon Resource Name (ARN) of the log group.

    This version of the ARN includes a trailing ``:*`` after the log group name.
    """
    ...
    inheritedProperties: "builtins.list[Literal['ACCOUNT_DATA_PROTECTION']]" = Field(
    ...
    This version of the ARN doesn't include a trailing ``:*`` after the log group name.
    """
    ...
    bearerTokenAuthenticationEnabled: bool = Field(default=None, frozen=True)
    """
    ...
    def name(self) -> str | None:
        ...
        This is the value of the :py:attr:`logGroupName` attribute.
...
class DescribeLogGroupsResponse(Boto3Model):
    logGroups: "builtins.list[LogGroup] | None" = Field(default_factory=list)
    """
...
class GetLogGroupFieldsResponse(Boto3Model):
    logGroupFields: "builtins.list[LogGroupField] | None" = Field(default_factory=list)
    """
