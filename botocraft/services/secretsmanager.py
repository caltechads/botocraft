...
class SecretManager(Boto3ModelManager):
    ...
    def create(
        ...
        encrypted form in Secrets Manager. The secret also includes the
        connection information to access a database or other service, which Secrets
        Manager doesn't encrypt. A secret in Secrets
        Manager consists of both the protected secret data and the important information
        ...
        Keyword Args:
            ClientRequestToken: If you include ``SecretString`` or ``SecretBinary``,
            ...
            SecretBinary: The binary data to encrypt and store in the new version of the
                secret. We recommend that you store
                your binary data in a file and then pass the contents of the file as a
                parameter.
            SecretString: The text data to encrypt and store in this new version of the
                secret. We recommend you use a JSON
    ...
    def update(
        ...
        Keyword Args:
            ...
            SecretBinary: The binary data to encrypt and store in the new version of the
                ...
                your binary data in a file and then pass the contents of the file as a
                parameter.
            SecretString: The text data to encrypt and store in the new version of the
                secret. We recommend you use a JSON
                structure of key/value pairs for your secret value.
        """
        data = model.model_dump(exclude_none=True, by_alias=True)
        ...
        *,
        ...
        Keyword Args:
            IncludePlannedDeletion: Specifies whether to include secrets scheduled for
                deletion. By default, secrets scheduled
    ...
    def delete(
        ...
        *,
        ...
        the recovery window, Secrets Manager
        ...
        Keyword Args:
            RecoveryWindowInDays: The number of days from 7 to 30 that Secrets Manager
                waits before permanently deleting the
                secret. You can't use both this parameter and
            ...
            ForceDeleteWithoutRecovery: Specifies whether to delete the secret without
                any recovery window. You can't use both
                this parameter and ``RecoveryWindowInDays`` in the same call. If you
                don't use either, then by default Secrets
    ...
    def set_value(
        ...
        *,
        ...
        Keyword Args:
            ...
            SecretBinary: The binary data to encrypt and store in the new version of the
                secret. To use this parameter in the
                command-line tools, we recommend that you store your binary data in a
                file and then pass the contents of the file as
                a parameter.
            SecretString: The text to encrypt and store in the new version of the
                secret.
            VersionStages: A list of staging labels to attach to this version of the
                secret. Secrets Manager uses staging labels
                to track versions of a secret through the rotation process.
            RotationToken: A unique identifier that indicates the source of the request.
                Required for secret rotations using an
                IAM assumed role or cross-account rotation, in which you rotate a secret
                in one account by using a Lambda rotation
                function in another account. In both cases, the rotation function
                assumes an IAM role to call Secrets Manager, and
    ...
    def get_value(
        ...
        *,
        ...
        Args:
            SecretId: The ARN or name of the secret to retrieve. To retrieve a secret
                from another account, you must use an ARN.

        Keyword Args:
            VersionId: The unique identifier of the version of the secret to retrieve.
                If you include both this parameter and
                ``VersionStage``, the two parameters must refer to the same secret
                version. If you don't specify either a
    ...
    def set_resource_policy(
        ...
        *,
        ...
        A resource-based policy is optional. For more information, see `Authentication
        and access control for Secrets
        ...
        Args:
            ...
            ResourcePolicy: A JSON-formatted string for an Amazon Web Services
                resource-based policy. For example policies, see
                `Permissions policy examples <https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-
                access_examples.html>`_.

        Keyword Args:
            BlockPublicPolicy: Specifies whether to block resource-based policies that
                allow broad access to the secret, for
                example those that use a wildcard for the principal. By default, public
    ...
    def get_resource_policy(self, SecretId: str) -> "GetResourcePolicyResponse":
        ...
        Args:
            SecretId: The ARN or name of the secret to retrieve the attached
    ...
    def delete_resource_policy(self, SecretId: str) -> "DeleteResourcePolicyResponse":
        ...
        Args:
            SecretId: The ARN or name of the secret to delete the attached
                resource-based policy for.
        """
        args: dict[str, Any] = dict(SecretId=self.serialize(SecretId))
    ...
    def rotate(
        ...
        *,
        ...
        For information about rotation, see `Rotate secrets <https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotating-secrets.html>`_ in the *Secrets Manager User
        Guide*. If you include the configuration parameters, the operation sets the
        values for the secret and then immediately
        starts a rotation. If you don't include the configuration parameters, the
        operation starts a rotation with the values
        ...
        Keyword Args:
            ClientRequestToken: A unique identifier for the new version of the secret.
                You only need to specify this value if
                you implement your own retry logic and you want to ensure that Secrets
                Manager doesn't attempt to create a secret
                version twice.
            RotationLambdaARN: For secrets that use a Lambda rotation function to
                rotate, the ARN of the Lambda rotation
                function.
            RotationRules: A structure that defines the rotation configuration for this
                secret.
            ExternalSecretRotationMetadata: The metadata needed to successfully rotate a
                managed external secret. A list of key
                value pairs in JSON format specified by the partner. For more
            ...
            ExternalSecretRotationRoleArn: The Amazon Resource Name (ARN) of the role
                that allows Secrets Manager to rotate a
                secret held by a third-party partner. For more information, see
                `Security and permissions
                <https://docs.aws.amazon.com/secretsmanager/latest/userguide/mes- security.html>`_.
            RotateImmediately: Specifies whether to rotate the secret immediately or
                wait until the next scheduled rotation
                window. The rotation schedule is defined in
    ...
    def random_password(
        ...
        *,
        ...
        and include every character type that the
        system you are generating a password for can support. By default, Secrets
        Manager uses uppercase and lowercase letters,
        numbers, and the following characters in passwords: ````
            !"#$%&'()*+,-./:;<=>?@[]^_``{|}~ ````

        Keyword Args:
            PasswordLength: The length of the password. If you don't include this
                parameter, the default length is 32
                characters.
            ExcludeCharacters: A string of the characters that you don't want in the
                password.
            ExcludeNumbers: Specifies whether to exclude numbers from the password. If
                you don't include this switch, the
                password can contain numbers.
            ExcludePunctuation: Specifies whether to exclude the following punctuation
                characters from the password: ```` ! " #
                $ % & ' ( ) * + , - . / : ; < = > ? @ [  ] ^ _ `` { | } ~ ````. If you
                    don't include this switch, the password can
                    contain punctuation.
            ExcludeUppercase: Specifies whether to exclude uppercase letters from the
                password. If you don't include this
                switch, the password can contain uppercase letters.
            ExcludeLowercase: Specifies whether to exclude lowercase letters from the
                password. If you don't include this
                switch, the password can contain lowercase letters.
            IncludeSpace: Specifies whether to include the space character. If you
                include this switch, the password can contain
                space characters.
            RequireEachIncludedType: Specifies whether to include at least one upper and
                lowercase letter, one number, and one
                punctuation. If you don't include this switch, the password contains at
    ...
    def update_version_stage(
        ...
        *,
        ...
        version at a time. To
        add a staging label to a version when it is already attached to another version,
        Secrets Manager first removes it from
        the other version first and then attaches it to this one. For more information
        about versions and staging labels, see
        `Concepts: Version <https://docs.aws.amazon.com/secretsmanager/latest/userguide/getting-started.html#term_version>`_.

        Args:
            SecretId: The ARN or the name of the secret with the version and staging
                labelsto modify.
            VersionStage: The staging label to add to this version.

        Keyword Args:
            RemoveFromVersionId: The ID of the version that the staging label is to be
                removed from. If the staging label you
                are trying to attach to one version is already attached to a different
                version, then you must include this parameter
                and specify the version that the label is to be removed from. If the
                label is attached and you either do not specify
                this parameter, or the version ID does not match, then the operation
                fails.
            MoveToVersionId: The ID of the version to add the staging label to. To
                remove a label from a version, then do not
...
class SecretVersionManager(Boto3ModelManager):
    ...
    def list(
        ...
        *,
        ...
        secret. For more information, see  `Secrets Manager concepts:
        ...
        Keyword Args:
            ...
            NextToken: A token that indicates where the output should continue from, if
                a previous call did not show all
                results. To get the next results, call ``ListSecretVersionIds`` again
                with this value.
            IncludeDeprecated: Specifies whether to include versions of secrets that
                don't have any staging labels attached to
                them. Versions without staging labels are considered deprecated and are
                subject to deletion by Secrets Manager. By
...
class RotationRulesType(Boto3Model):
    ...
    You can use this value to check that your secret meets your compliance guidelines
    for how often secrets must be rotated. If you use this field to set the rotation
    schedule, Secrets Manager calculates the next rotation date based on the previous
    rotation. Manually updating the secret value by calling ``PutSecretValue`` or
    ...
    ``ScheduleExpression`` in hours, the window automatically closes after one hour. For
    a ``ScheduleExpression`` in days, the window automatically closes at the end of the
    UTC day. For more information, including examples, see
    `Schedule expressions in Secrets Manager rotation <https://docs.aws.amazon.com/secretsmanager/latest/userguide/rotate-secrets_schedule.html>`_
    in the *Secrets
    Manager Users Guide*.
    """
...
class Secret(TagsDictMixin, PrimaryBoto3Model):
    ...
    It does not include the encrypted ``SecretString`` and ``SecretBinary`` values. To
    get those values, use
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = SecretManager
...
```RotateSecret`` <https://docs.aws.amazon.com/secretsmanager/latest/apireference/API_RotateSecret.html>`_ .
    ...
    Not present on active secrets. The secret can be recovered until the number of days
    in the recovery window has passed, as specified in the ``RecoveryWindowInDays``
    parameter of the
    ...
    To add tags to a secret, use ```TagResource`` <https://docs.aws.amazon.com/secretsmanager/latest/apireference/API_TagResource.html>`_ . To remove tags,
...
class SecretVersion(PrimaryBoto3Model):
    ...
    """
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = SecretVersionManager
    VersionId: "str | None" = None
    """
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`VersionId` attribute.
...
class DescribeSecretResponse(TagsDictMixin, Boto3Model):
    ...
    If the secret is encrypted with the Amazon Web Services managed key
    ``aws/secretsmanager``, this field is omitted. Secrets created using the console use
    an KMS key ID.
    """
...
class ListSecretsResponse(Boto3Model):
    ...
    This can occur even when the response includes no values at all, such as when you
    ask for a filtered view of a long list. To get the next results, call
...
class GetSecretValueResponse(Boto3Model):
    ...
    When you retrieve a ``SecretBinary`` using the HTTP API, the Python SDK, or the
    ...
    If you don't specify which version in ``VersionId`` or ``VersionStage``, then
...
class CancelRotateSecretResponse(Boto3Model):
    ...
    We recommend that you remove the ``VersionStage`` value ``AWSPENDING`` from this
    version so that Secrets Manager can delete it. Failing to clean up a cancelled
