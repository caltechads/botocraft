...
class PipelineManager(Boto3ModelManager):
    ...
    def update(self, model: "Pipeline") -> "UpdatePipelineOutput":
        ...
        Use a JSON file with the pipeline structure and ``UpdatePipeline`` to provide
    ...
    def get(
        ...
        Args:
            name: The name of the pipeline for which you want to get information.
                Pipeline names must be unique in an Amazon Web
                Services account.

        Keyword Args:
            version: The version number of the pipeline. If you do not specify a
                version, defaults to the current version.
        """
    ...
    def list(self, *, version: "int | None" = None) -> PrimaryBoto3ModelQuerySet:
        """
        Returns every pipeline in the account.

        Each entry is loaded with ````GetPipeline```` after listing, so models include
        the full definition (stages, artifact stores, variables, triggers,
        ````pipelineArn````, etc.), not only
        ````ListPipelines```` summary fields. Optional ````version```` pins the pipeline
        revision fetched for every name; omit
        it to load the latest revision for each pipeline.

        Keyword Args:
            version: When set, each pipeline is loaded with ````GetPipeline```` using
                this version number. When omitted, the
    ...
    def start_execution(
        ...
        *,
        ...
        Keyword Args:
            variables: A list that overrides pipeline variables for a pipeline execution
                that's being started. Variable names
                must match ``[A-Za-z0-9@-_]+``, and the values can be anything except an
                empty string.
            clientRequestToken: The system-generated unique ID used to identify a unique
                execution request.
            sourceRevisions: A list that allows you to specify, or override, the source
                revision for a pipeline execution that's
                being started. A source revision is the version with all the changes to
                your application code, or source artifact,
    ...
    def stop_execution(
        ...
        *,
        ...
        actions. While completing or abandoning in-
        progress actions, the pipeline execution is in a ``Stopping`` state. After all
        in-progress actions are completed or
        abandoned, the pipeline execution is in a ``Stopped`` state.

        Args:
            pipelineName: The name of the pipeline to stop.
            pipelineExecutionId: The ID of the pipeline execution to be stopped in the
                current stage. Use the
                ``GetPipelineState`` action to retrieve the current pipelineExecutionId.

        Keyword Args:
            abandon: Use this option to stop the pipeline execution by abandoning,
                rather than finishing, in-progress actions.
            reason: Use this option to enter comments, such as the reason the pipeline
                was stopped.
        """
        args: dict[str, Any] = dict(
...
class PipelineExecutionManager(ReadonlyBoto3ModelManager):
    ...
    def get(
        ...
        Args:
            pipelineName: The name of the pipeline about which you want to get execution
                details.
            pipelineExecutionId: The ID of the pipeline execution about which you want
    ...
    def list(
        ...
        Args:
            pipelineName: The name of the pipeline for which you want to get execution
                summary information.
...
class ActionTypeManager(Boto3ModelManager):
    ...
    def delete(
        ...
        Args:
            category: The category of the custom action that you want to delete, such as
                source or deploy.
    ...
    def get(
        ...
        Args:
            category: Defines what kind of action can be taken in the stage. The
                following are the valid values:
            owner: The creator of an action type that was created with any supported
                integration model. There are two valid
                values: ``AWS`` and ``ThirdParty``.
            provider: The provider of the action type being called. The provider name is
                specified when the action type is
                created.
    ...
    def list(
        ...
        *,
        ...
        Keyword Args:
            actionOwnerFilter: Filters the list of action types to those created by a
                specified entity.
...
class ActionTypeId(Boto3Model):
    ...
    information, see
    `Valid Action Types and Providers in CodePipeline <https://docs.aws.amazon.com/codepipeline/latest/userguide/reference-pipeline-
    ...
    `Valid Action Types and Providers in CodePipeline <https://docs.aws.amazon.com/codepipeline/latest/userguide/reference-pipeline-structure.html#actions-valid-
    providers>`_.
    """
    version: str
    """
...
class ActionDeclaration(Boto3Model):
    ...
    These are key-value pairs that specify input values for an action. For more information, see `Action Structure Requirements in CodePipeline <https://docs.aws.amazon.com/codepipeline/latest/userguide/reference-
    pipeline-structure.html#action-requirements>`_. For the list of configuration
    properties for the CloudFormation action
    ...
    reference.html>`_ in the *CloudFormation User Guide*. For template snippets with
    examples, see `Using Parameter Override
    Functions with CodePipeline Pipelines <https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/continuous-
    delivery-codepipeline-parameter-override-functions.html>`_ in the *CloudFormation
    User Guide*.
    """
...
timeout value specified in `Quotas
...
class Condition(Boto3Model):
    ...
    A condition is made up of the rules and the result for the condition. For more
    information about conditions, see
    `Stage conditions <https://docs.aws.amazon.com/codepipeline/latest/userguide/stage-
     conditions.html>`_ and
    `How do stage conditions work? <https://docs.aws.amazon.com/codepipeline/latest/userguide/concepts-
     how-it-works-conditions.html>`_.. For more information about rules, see the
    `CodePipeline rule reference <https://docs.aws.amazon.com/codepipeline/latest/userguide/rule-reference.html>`_.
    """
...
class GitPushFilter(TagsDictMixin, Boto3Model):
    ...
    tag_class: ClassVar[ModelType[Boto3Model]] = GitTagFilterCriteria
...
class GitConfiguration(Boto3Model):
    ...
    source actions that are supported by the ``CodeStarSourceConnection`` action type.
    """
...
class SourceRevisionOverride(Boto3Model):
    ...
    execution that's being started.

    A source revision is the version with all the changes to your application code, or
...
class Pipeline(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = PipelineManager
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`pipelineName` attribute.
...
class ArtifactRevision(Boto3Model):
    ...
    For GitHub and CodeCommit repositories, the commit message. For Amazon S3 buckets or
...
class PipelineExecution(ReadonlyPrimaryBoto3Model):
    ...
    """
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = PipelineExecutionManager
...
class ActionType(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = ActionTypeManager
...
class ActionTypeArtifactDetails(Boto3Model):
    ...
    For example, you should specify a minimum and maximum of zero input artifacts for an
