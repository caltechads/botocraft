...
class EBSTagSpecification(TagsDictMixin, Boto3Model):
    ...
    ``SERVICE`` specified in ``ServiceVolumeConfiguration``. If no value is specified, the tags aren't propagated.
    """
...
class DeploymentLifecycleHookDetail(Boto3Model):
    ...
    For ``AWS_LAMBDA`` hooks, this is the Lambda function ARN. For
    ``PAUSE`` hooks, this field is not set.
    """
