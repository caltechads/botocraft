...
class LoadBalancerManager(LoadBalancerManagerMixin, Boto3ModelManager):
    ...
    def create(
        ...
        Keyword Args:
            SubnetMappings: The IDs of the subnets. You can specify only one subnet per
                Availability Zone. You must specify
    ...
    def list(
        ...
        *,
        ...
        Keyword Args:
            LoadBalancerArns: The Amazon Resource Names (ARN) of the load balancers. You
                can specify up to 20 load balancers in
                a single call.
...
class ListenerManager(Boto3ModelManager):
    ...
    def update(self, model: "Listener") -> "builtins.list[Listener]":
        """
    ...
    def get(self, ListenerArn: str) -> "Listener | None":
        """
...
class RuleManager(Boto3ModelManager):
    ...
    def update(
        ...
        Any properties that you do not specify are unchanged.
                ...
                Keyword Args:
                    ResetTransforms: Indicates whether to remove all transforms from the
                        rule. If you specify ``ResetTransforms``, you
    ...
    def get(self, RuleArn: str) -> "Rule | None":
        """
...
class TargetGroupManager(Boto3ModelManager):
    ...
    def update(self, model: "TargetGroup") -> "builtins.list[TargetGroup]":
        """
    ...
    def get(
        ...
        By default, all target groups are described. Alternatively, you can specify one of the following to filter the results: the
            ARN of the load balancer, the names of
...
class LoadBalancerState(Boto3Model):
    ...
    The initial state of the load balancer is ``provisioning``. After the load balancer
    is fully set up and ready to route traffic, its state is ``active``. If load
    balancer is routing traffic but does not have the resources it needs to scale, its
    state is``active_impaired``. If the load balancer could not be set up, its state is
    ``failed``.
    """
...
class LoadBalancer(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = LoadBalancerManager
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`LoadBalancerArn` attribute.
    ...
    def name(self) -> str | None:
        ...
        This is the value of the :py:attr:`LoadBalancerName` attribute.
    ...
    def attributes(self) -> "builtins.list[LoadBalancerAttribute]":
        """
...
class AuthenticateOidcActionConfig(Boto3Model):
    ...
    This parameter is required if you are creating a rule. If you are modifying a rule,
...
class RedirectActionConfig(Boto3Model):
    ...
    A URI consists of the following components: protocol://hostname:port/path?query. You
        must modify at least one of the
    following components to avoid a redirect loop: protocol, hostname, port, or path.
        Any components that you do not modify
...
class JwtValidationActionAdditionalClaim(Boto3Model):
    ...
    You can't specify ``exp``, ``iss``, ``nbf``, or ``iat`` because we validate them by
    default.
    """
    ...
    The maximum size of the list is 10. Each value can be up to 256 characters in
    length. If the format is ``space-separated-values``, the values can't include
    spaces.
    """
...
class Action(Boto3Model):
    ...
    Specify only when ``Type`` is ``forward`` and you want to route to a single target
    group. To route to multiple target groups, you must use ``ForwardConfig`` instead.
    """
...
class Listener(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = ListenerManager
    LoadBalancerArn: str
    """
    The Amazon Resource Name (ARN) of the load balancer.
    """
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`ListenerArn` attribute.
...
class RuleCondition(Boto3Model):
    ...
    header``, ``path-pattern``, and ``source-ip``. Each rule can also optionally include
    ...
    Specify only when ``Field`` is ``host-header`` or ``path-pattern``. Alternatively,
    to specify multiple host names or multiple path patterns, use ``HostHeaderConfig``
    ...
    The maximum length of each string is 128 characters. Specify only when ``Field`` is
    ``http-header``, ``host-header``, or ``path-pattern``.
    """
...
class Rule(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = RuleManager
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`RuleArn` attribute.
...
class TargetDescription(Boto3Model):
    ...
    If the target type of the target group is ``instance``, specify an instance ID. If
    the target type is ``ip``, specify an IP address. If the target type is ``lambda``,
    specify the ARN of the Lambda function. If the target type is ``alb``, specify the
    ...
    This value is required if the protocol is ``QUIC`` or ``TCP_QUIC`` and can't be used
...
class TargetGroup(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = TargetGroupManager
    TargetGroupName: str
    """
    ...
    The possible values are ``instance`` (register targets by instance ID), ``ip``
    (register targets by IP address), ``lambda`` (register a single Lambda function as a
    target), or ``alb`` (register a single Application Load Balancer as a target).
    """
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`TargetGroupArn` attribute.
    ...
    def name(self) -> str | None:
        ...
        This is the value of the :py:attr:`TargetGroupName` attribute.
    ...
    def vpc(self) -> "Vpc | None":
        ...
        try:
            ...
            return None
        return Vpc.objects.using(self.session).get(**pk)  # type: ignore[arg-type]
    def targets(self) -> "builtins.list[TargetHealthDescription] | None":
        """
