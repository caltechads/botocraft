...
class HostedZoneManager(Boto3ModelManager):
    ...
    def create(
        ...
        HostedZoneConfig: "HostedZoneConfig | None" = None,
        ...
        (apex.example.com, acme.example.com). You
        create records in a private hosted zone to define how you want to route traffic
        for a domain and its subdomains within
        ...
        Keyword Args:
            VPC: (Private hosted zones only) A complex type that contains information
                about the Amazon VPC that you're
            ...
            DelegationSetId: If you want to associate a reusable delegation set with
                this hosted zone, the ID that Amazon
                Route 53 assigned to the reusable delegation set when you created it.
                For more information about reusable delegation
    ...
    def get(self, Id: str) -> "HostedZone | None":
        """
    ...
    def list(
        ...
        *,
        ...
        Keyword Args:
            MaxItems: (Optional) The maximum number of hosted zones that you want Amazon
                Route 53 to return. If you have more
                than ``maxitems`` hosted zones, the value of ``IsTruncated`` in the
                response is ``true``, and the value of
                ``NextMarker`` is the hosted zone ID of the first hosted zone that Route
                53 will return if you submit another
                request.
            DelegationSetId: If you're using reusable delegation sets and you want to
                list all of the hosted zones that are
    ...
    def list_records(
        ...
        *,
        ...
        MaxItems: "str | None" = None,
    ) -> "builtins.list[Route53ResourceRecordSet]":
        """
        ...
        Keyword Args:
            StartRecordName: The first name in the lexicographic ordering of resource
                record sets that you want to list. If the
                specified record name doesn't exist, the results begin with the first
                resource record set that has a name greater
                than the value of ``name``.
            StartRecordType: The type of resource record set to begin the record listing
                from.
            StartRecordIdentifier: *Resource record sets that have a routing policy
                other than simple:* If results were
                truncated for a given DNS name and   type, specify the value of
                ``NextRecordIdentifier`` from the previous response
            ...
            MaxItems: (Optional) The maximum number of resource records sets to include
                ...
                the value of the ``IsTruncated`` element in
                the response is ``true``, and the values of the ``NextRecordName`` and
                ``NextRecordType`` elements in the response
        ...
        results: "builtins.list[Route53ResourceRecordSet]" = []
        ...
        self.sessionize(results)
        return cast("builtins.list[Route53ResourceRecordSet]", results)
        ...
        *,
        ...
        Keyword Args:
            DNSName: (Optional) For your first request to ``ListHostedZonesByName``,
                include the ``dnsname`` parameter only if
                you want to specify the name of the first hosted zone in the response.
                If you don't include the ``dnsname``
                parameter, Amazon Route 53 returns all of the hosted zones that were
                created by the current Amazon Web Services
            ...
            HostedZoneId: (Optional) For your first request to
                ``ListHostedZonesByName``, do not include the ``hostedzoneid``
                parameter.
            MaxItems: The maximum number of hosted zones to be included in the response
                body for this request. If you have more
                than ``maxitems`` hosted zones, then the value of the ``IsTruncated``
                element in the response is true, and the
                values of ``NextDNSName`` and ``NextHostedZoneId`` specify the first
        ...
        results: "builtins.list[HostedZone]" = None
        if response is not None:
            results = response.HostedZones

        self.sessionize(results)
        return cast("builtins.list[HostedZone]", results)
    ...
    def update_comment(self, Id: str, Comment: str) -> "HostedZone":
        ...
        Args:
            ...
            Comment: The new comment for the hosted zone. If you don't specify a value
                for ``Comment``, Amazon Route 53 deletes
    ...
    def associate_vpc(
        ...
        Args:
            HostedZoneId: The ID of the private hosted zone that you want to associate
                an Amazon VPC with.
            VPC: A complex type that contains information about the VPC that you want to
    ...
    def disassociate_vpc(
        ...
        Args:
            ...
            VPC: A complex type that contains information about the VPC that you're
                disassociating from the specified hosted
                zone.
...
class Route53VPCManager(Boto3ModelManager):
        ...
        *,
    ...
    ) -> "builtins.list[HostedZoneSummary]":
        """
        Lists all the private hosted zones that a specified VPC is associated with,
        regardless of which Amazon Web Services
        account or Amazon Web Services service owns the hosted zones. The
        ``HostedZoneOwner`` structure in the response contains
        ...
        Args:
            ...
            VPCRegion: For the Amazon VPC that you specified for ``VPCId``, the Amazon
                Web Services Region that you created the
                VPC in.

        Keyword Args:
            MaxItems: (Optional) The maximum number of hosted zones that you want Amazon
                Route 53 to return. If the specified
                VPC is associated with more than ``MaxItems`` hosted zones, the response
                includes a ``NextToken`` element.
                ``NextToken`` contains an encrypted token that identifies the first
                hosted zone that Route 53 will return if you
                submit another request.
            NextToken: If the previous response included a ``NextToken`` element, the
                specified VPC is associated with more
                hosted zones. To get more hosted zones, submit another
        ...
        results: "builtins.list[HostedZoneSummary]" = None
        if response is not None:
            results = response.HostedZoneSummaries

        self.sessionize(results)
        return cast("builtins.list[HostedZoneSummary]", results)

    def list_authorizations(self, HostedZoneId: str) -> "builtins.list[Route53VPC]":
        """
        ...
        Args:
            HostedZoneId: The ID of the hosted zone for which you want a list of VPCs
                that can be associated with the hosted
                zone.
        """
        ...
        results: "builtins.list[Route53VPC]" = []
        ...
        self.sessionize(results)
        return cast("builtins.list[Route53VPC]", results)
    ...
    def create_authorization(
        ...
        Args:
            ...
            VPC: A complex type that contains the VPC ID and region for the VPC that you
                want to authorize associating with your
                hosted zone.
        """
        args: dict[str, Any] = dict(
    ...
    def delete_authorization(
        ...
        Args:
            HostedZoneId: When removing authorization to associate a VPC that was
                created by one Amazon Web Services account
                with a hosted zone that was created with a different Amazon Web Services
                account, the ID of the hosted zone.
            VPC: When removing authorization to associate a VPC that was created by one
                Amazon Web Services account with a
                hosted zone that was created with a different Amazon Web Services
                account, a complex type that includes the ID and
                region of the VPC.
        """
        args: dict[str, Any] = dict(
            HostedZoneId=self.serialize(HostedZoneId), VPC=self.serialize(VPC)
...
class Route53CidrCollectionManager(Boto3ModelManager):
    ...
    def create(
        ...
        Args:
            ...
            CallerReference: A client-specific token that allows requests to be securely
                retried so that the intended outcome
                will only occur once, retries receive a similar response, and there are
    ...
    def update(
        ...
        Keyword Args:
            CollectionVersion: A sequential counter that Amazon Route 53 sets to 1 when
                you create a collection and increments
...
class Route53QueryLoggingConfigManager(Boto3ModelManager):
    ...
    def get(self, Id: str) -> "Route53QueryLoggingConfig | None":
        """
    ...
    def list(self, *, HostedZoneId: "str | None" = None) -> PrimaryBoto3ModelQuerySet:
        ...
        Keyword Args:
            HostedZoneId: (Optional) If you want to list the query logging configuration
                that is associated with a hosted zone,
...
class HostedZone(HostedZoneModelMixin, PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = HostedZoneManager
    ...
    If you omitted the ``HostedZoneConfig`` and ``Comment`` elements from the request,
...
class Route53VPC(PrimaryBoto3Model):
    ...
    `CreateHostedZone <https://docs.aws.amazon.com/Route53/latest/APIReference/API_CreateHostedZone.html>`_ request, the
    following parameters are also required.
    """

    manager_class: ClassVar[ModelType[Boto3ModelManager]] = Route53VPCManager
...
class Route53CidrCollection(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = Route53CidrCollectionManager
...
class CidrBlockSummary(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = CidrBlockSummaryManager
    ...
    def pk(self) -> str | None:
        ...
        This is the value of the :py:attr:`CidrBlock` attribute.
...
class Route53QueryLoggingConfig(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = (
        Route53QueryLoggingConfigManager
    )
...
class Route53GeoLocation(Boto3Model):
    ...
    Route 53 doesn't support any other values for ``SubdivisionCode``. For a list of state abbreviations, see `Appendix B: Two-Letter
        State and
...
class ResourceRecord(Boto3Model):
    ...
    In the case of a ``DELETE`` action, if the current value does not match the actual
    value, an error is returned. For descriptions about how to format ``Value`` for
    different record types, see
...
class Route53AliasTarget(Boto3Model):
    ...
    * For information about creating failover resource record sets in a private hosted
    zone, see `Configuring Failover in a
...
class Route53GeoProximityLocation(Boto3Model):
    ...
    Only one of ``LocalZoneGroup``, ``Coordinates``, or ``Amazon Web ServicesRegion`` is
...
class Route53ResourceRecordSet(PrimaryBoto3Model):
    ...
    manager_class: ClassVar[ModelType[Boto3ModelManager]] = (
        Route53ResourceRecordSetManager
    )
    ...
    For
    ``ListResourceRecordSets`` responses, the name of a record in the specified hosted
    zone.
    """
    ...
    *Resource record sets that have a routing policy other than simple:* An identifier
    ...
    In a group of resource record sets that have the same name and type, the value of
    ...
    *Geolocation resource record sets only:* A complex type that lets you control how
    ...
    For example, if you want all queries from Africa to be routed to a web server with
    an IP address of ``192.0.2.111``, create a resource record set with a ``Type`` of
    ``A`` and a ``ContinentCode`` of ``AF``.
    """
    ...
    *Failover resource record sets only:* To configure failover, you add the
    ...
    for the other resource record set, you specify ``SECONDARY``. In addition, you
    include the ``HealthCheckId`` element and specify the health check that you want
    ...
    *Multivalue answer resource record sets only*: To route traffic approximately
    ...
    ResourceRecords: "builtins.list[ResourceRecord]" = Field(
    ...
    *Alias resource record sets only:* Information about the Amazon Web Services
    ...
    ``TrafficPolicyInstanceId`` is the ID of the traffic policy instance that Route 53
...
class GetHostedZoneResponse(Boto3Model):
    ...
    VPCs: "builtins.list[Route53VPC] | None" = Field(default_factory=list)
    """
...
class ListHostedZonesResponse(Boto3Model):
    ...
    If the response was truncated, you can get more hosted zones by submitting another
    ...
    Submit another ``ListHostedZones`` request, and specify the value of ``NextMarker``
...
class ListResourceRecordSetsResponse(Boto3Model):
    ...
    ResourceRecordSets: "builtins.list[Route53ResourceRecordSet]"
    """
    ...
    If your results were truncated, you can make a follow-up pagination request by using
    the ``NextRecordName`` element.
    """
...
class ListHostedZonesByNameResponse(Boto3Model):
    ...
    If the response was truncated, you can get the next group of ``maxitems`` hosted
...
class GetHostedZoneLimitResponse(Boto3Model):
    ...
    the request, the value of ``Limit`` is the maximum number of records that you can
    ...
    For example, if you specified ``MAX_RRSETS_BY_ZONE`` for the value of ``Type`` in
    the request, the value of ``Count`` is the current number of records that you have
...
class HostedZoneOwner(Boto3Model):
    ...
    If there is a value for ``OwningAccount``, there is no value for ``OwningService``,
    and vice versa.
    """
    ...
    For example, if Amazon Elastic File System (Amazon EFS) created a hosted zone and
...
class HostedZoneSummary(Boto3Model):
    ...
    Each
    ``HostedZoneSummary`` element contains the hosted zone name and ID, and information
    about who owns the hosted zone.
    """
...
class ListHostedZonesByVPCResponse(Boto3Model):
    HostedZoneSummaries: "builtins.list[HostedZoneSummary]"
    """
    A list that contains one ``HostedZoneSummary`` element for each hosted zone that the
    specified Amazon VPC is associated with.

    Each ``HostedZoneSummary`` element contains the hosted zone name and ID, and
...
class ListCidrCollectionsResponse(Boto3Model):
    ...
    CidrCollections: "builtins.list[CollectionSummary] | None" = Field(
...
class ChangeCidrCollectionResponse(Boto3Model):
    ...
    You can use it as input to ``GetChange`` to see if a CIDR collection change has
    propagated or not.
    """
...
class ListCidrBlocksResponse(Boto3Model):
    ...
    CidrBlocks: "builtins.list[CidrBlockSummary] | None" = Field(default_factory=list)
    """
...
class ListQueryLoggingConfigsResponse(Boto3Model):
    QueryLoggingConfigs: "builtins.list[Route53QueryLoggingConfig]"
    """
An array that contains one
`QueryLoggingConfig <https://docs.aws.amazon.com/Route53/latest/APIReference/API_QueryLoggingConfig.html>`_ element for
each configuration for DNS query logging that is associated with the current Amazon Web
Services account.
    """
