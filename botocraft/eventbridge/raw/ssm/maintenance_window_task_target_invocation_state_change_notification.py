...
class Detail(BaseModel):
    ...
    #: Event field ``task-execution-id``.
    task_execution_id: str = Field(..., alias="task-execution-id")
    #: Event field ``window-target-id``.
    window_target_id: str = Field(..., alias="window-target-id")
    #: Event field ``status``.
    status: str
    #: Event field ``owner-information``.
    owner_information: str = Field(..., alias="owner-information")


class SSMMaintenanceWindowTaskTargetInvocationStateChangeNotificationEvent(BaseModel):
    """
